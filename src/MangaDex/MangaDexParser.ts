...
export const parseMangaList = async (object: any, source: any, thumbnailSelector: any): Promise<PartialSourceManga[]> => {
...
export const parseChapterList = async (chapterObject: any, coversMapping: {
    [id: string]: string;
}, source: any, thumbnailSelector: any, ratings: string[]): Promise<PartialSourceManga[]> => {
    const results: PartialSourceManga[] = []
    for (const chapter of chapterObject) {
        ...
        const image = coverFileName ? `${source.COVER_BASE_URL}/${mangaId}/${coverFileName}${MDImageQuality.getEnding(await thumbnailSelector(source.stateManager))}` : 'https://mangadex.org/_nuxt/img/cover-placeholder.d12c3c5.jpg'
        if (!mangaId || !title || !ratings.includes(chapter.relationships.filter((x: any) => x.type == 'manga').map((x: any) => x.attributes.contentRating)[0]))
            continue
        results.push(App.createPartialSourceManga({
            title: title,
            image: image,
